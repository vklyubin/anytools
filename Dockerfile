...
RUN microdnf install -y tar openssl nginx mlocate bash bash-completion openssh-clients nmap tcpdump bind-utils vim less net-tools iputils iproute ca-certificates traceroute frr openldap-clients curl && \
    microdnf clean all
RUN echo "TLS_REQCERT never" >> /etc/openldap/ldap.conf
COPY ca-vf-root-ca.pem /etc/pki/ca-trust/source/anchors/ca-vf-root-ca.pem
RUN update-ca-trust
